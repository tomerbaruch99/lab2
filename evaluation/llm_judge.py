...
def build_judge_prompt(question: str, gold_answer: str, rag_answer: str) -> str:
    ...
    """
    return f"""
אתה בוחן איכות תשובות של צ'אטבוט המתמחה באתר האינטרנט של עיריית חיפה.

השאלה שהמשתמש שלח לצ'אטבוט:
\"\"\"{question}\"\"\"

התשובה הנכונה מתוך אתר האינטרנט הרשמי של עיריית חיפה:
\"\"\"{gold_answer}\"\"\"

התשובה שהחזיר הצ'אטבוט למשתמש:
\"\"\"{rag_answer}\"\"\"

הערך את התשובה של הצ'אטבוט ביחס לתשובה הנכונה לפי המדדים הבאים (ציון 0 עד 1):
